...
// Pass store to NewHealthHandler - QdrantStorage implements HealthChecker
// via its Health(ctx context.Context) error method
...
Important: `store` (from storage.NewQdrantStorage) satisfies HealthChecker interface because QdrantStorage has a `Health(ctx context.Context) error` method that pings Qdrant.
