...
must_haves:
  truths:
    ...
    - "Qdrant process has 1GB persistent volume mounted at /qdrant/storage"
    - "Health checks configured to verify /health endpoint"
    - "MCP server process configured with 256MB, Qdrant with 512MB"
  artifacts:
    - path: "fly.toml"
...
- QDRANT_HOST=localhost (same-host optimization per RESEARCH.md)
  ...
  <verify>
1. Validate fly.toml syntax: fly config validate (if flyctl installed)
2. Otherwise, manual review:
   - [processes] has web and qdrant
   - [[vm]] sections specify correct memory
   - [[mounts]] has processes = ["qdrant"]
